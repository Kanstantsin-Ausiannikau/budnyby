(function ($) {
	$.fn.chameleonSlider_1_5_1 = function (options) {
		var defaults = {
				content_source: '',
				...
				social_buttons: {
					...
					buttons: {
						...
						twitter: {
							display: true,
							html: '<a href="http://twitter.com/share" class="twitter-share-button" data-url="{location_href}" data-count="horizontal">Tweet</a><script type="text/javascript" src="//platform.twitter.com/widgets.js"></' + 'script>'
						}
					},
					hideable: false,
					container: {
			...
			slider = {
				init: function () {
					...
					if (option.container_dimensions.w_as_ratio || option.container_dimensions.h_as_ratio) {
						...
						rendering.container_dimensions = {
							...
							height: option.container_dimensions.h_as_ratio ? Math.round($slider_container.height() * option.container_dimensions.height) : option.container_dimensions.height
						};
						...
						$(window).bind('resize.chameleonSlider', function () {
							...
							slider_resize_interval = setTimeout(function () {
								var dimensions = {
										...
										height: option.container_dimensions.h_as_ratio ? $slider_container.height() : option.container_dimensions.height
									};

								$t.trigger('resize_slider', [dimensions]);
							}, 200);
						});
					} else
						rendering.container_dimensions = {
				...
				load: function(data) {
					...
					if (option.open_at != null) {
						...
						if (option.fullscreen_provider == 'smb'
							&& item_to_open != undefined
				...
				show_item: function (index, force, transition) {
						...
						if (option.main_panel.drag_navigation) {
							...
							$(document)
								.bind(mousemove + '.chameleon_' + option.module_id, function (e) {
									...
									position_delta = $main_panel.data('dragging').start_position.left - e.pageX;
					...
					_transitions: {
						show: function (item_index) {
							...
							if ((current_item.type == 'video' || current_item.type == 'audio') && item_html) {
								...
								next_item_html += '<img alt="" src="' + current_item.src + '" />';
							}
						...
						fade: function (item_index) {
							var $next_item = $('<div class="item_container"><img alt="" src="' + current.category.items[item_index].src + '" /></div>');
						...
						slide: function (item_index, fx_prop) {
							var $next_item = $('<div class="item_container new_item"><img alt="" src="' + current.category.items[item_index].src + '" /></div>'),
				...
				_video: {
					...
					_youtube_player: function (param) {
						var player_id = param.container.attr('id') + '_object',
							player_html = '<iframe id="' + player_id + '" type="text/html" width="' + param.dimensions.width + '" height="' + param.dimensions.height + '" ' +
								'src="http://www.youtube.com/embed/' + param.id + '?enablejsapi=1&amp;wmode=transparent&amp;origin=' + location.protocol + '//' + location.host + '" ' +
					...
					_vimeo_player: function (param) {
						var video_id = param.container.attr('id') + '_object',
							video_url = 'http://vimeo.com/moogaloop.swf?clip_id=' + param.id + '&amp;server=vimeo.com&amp;show_title=1&amp;show_byline=1&amp;show_portrait=1&amp;fullscreen=1&amp;autoplay=0&amp;loop=0&amp;api=1&amp;player_id=' + video_id,
							embed_template = '<object width="' + param.dimensions.width + '" height="' + param.dimensions.height + '">'+
								'<param name="movie" value="' + video_url + '" />'+
								'<param name="allowFullScreen" value="true" />'+
								'<param name="allowscriptaccess" value="always" />'+
								'<param name="wmode" value="transparent" />'+
								'<param name="flashvars" value="api=1" />'+
								'<embed id="' + video_id + '" src="' + video_url + '" type="application/x-shockwave-flash" wmode="transparent" width="' + param.dimensions.width + '" height="' + param.dimensions.height + '" allowscriptaccess="always" allowfullscreen="true"></embed>'+
								'</object>';

						window.chameleon_video_vimeo_play = function () {
								autoplay.video_playing = true;
								slider.autoplay.reset();
							};

						window.chameleon_video_vimeo_end = function () {
								setTimeout(slider.next, 1000);
							};

						window.vimeo_player_loaded = function (player_id) {
							var player = document.getElementById(player_id);

							$t.trigger('slider_transition_finnished');

							player.api_addEventListener('play', 'chameleon_video_vimeo_play');
							player.api_addEventListener('finish', 'chameleon_video_vimeo_end');
						}

						param.container.append(embed_template);
					},
							...
							if (box.jq.container.is(':visible')) {
								...
								box.jq.viewport.stop(true).fadeTo(200, 0, function () {
									...
									if (box_options.container.resize_to_content && (!box_options.container.show_on_hover || rendering.mouse_entered)) {
										...
										box.jq.container.height(box.jq.viewport.outerHeight(true));
										show_content();
									}
								});
							} else {
				...
				thumbnails: {
					init: function () {
						...
						slider._items_panel.init({
							panel: 'thumbs',
							page: $('<ul />'),
							item: $('<li><img src="' + slider_content[0].items[0].thumb.src + '" alt="" style="width: ' + option.thumbs.width + 'px; height: ' + option.thumbs.height + 'px;" /></li>')
						});
					...
					display_category: function () {
						...
						for (; i < current.category.item_count; i++) {
							...
							thumb_html = '<li><img style="width: ' + option.thumbs.width + 'px; height: ' + option.thumbs.height + 'px;" src="' + current.category.items[i].thumb.src + '" alt="" />' + thumb_type_html + thumb_caption;
				...
				triggers: function () {
					...
					$t.bind('resize_slider', function (e, dimensions) {
						slider.autoplay.reset();

						rendering.container_dimensions = {
							width: option.container_dimensions.w_as_ratio ? Math.round(dimensions.width * option.container_dimensions.width) : dimensions.width,
							height: option.container_dimensions.h_as_ratio ? Math.round(dimensions.height * option.container_dimensions.height) : dimensions.height
						}
				...
				position_element: function (element, position) {
					...
					if (position.v_center_point)
						v_offset -= Math.floor(element.height() / 2);

					if (position.h_center_point)
						h_offset -= Math.floor(element.width() / 2);
	...
	$.fn.chameleonSliderFullscreen = function (options) {
		...
		$theSlider.chameleonSlider_1_5_1(options);
	};
}(eds1_8));
